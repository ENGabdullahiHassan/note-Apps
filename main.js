...
function showNotes(n) {
  showNote.innerHTML = "";
  n.forEach((note) => {
    showNote.innerHTML += `  <div class="note addCard">
        <div class="details">
        <h2>${note.title}</h2>
        ...
        <div class="button-contant">
          <span> ${note.date}</span>
          <div class="setting">
              <button id="updateBtn" class="updateBtn"><i onclick="updateNote(${note.id}, '${note.title}','${note.description}')" class="fa-regular fa-pen-to-square"></i></button> 
              <button id="deleteBtn"><i class="deleteBtn" noteId="${note.id}"> Del</i></button>
